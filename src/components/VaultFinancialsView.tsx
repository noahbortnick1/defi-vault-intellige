...
vaultName }: VaultFinancialsViewProps) {
  const { balanceSheet, incomeStatement, flowOfFunds, navHistory, positionNotes } = financials;

      <Card>nt.incentiveIncome + 
        <CardHeader>ement.tradingFeeIncome + incomeStatement.stakingIncome;
          <div className="flex items-center justify-between">
            <div>atement.performanceFees;
        ...
        <CardContent className="space-y-8">
          <div>
            ...
            <div className="space-y-6">
              ...
              </div>eparator className="my-2" />
-between items-center py-1 font-semibold">
              <div> <span>Total Assets</span>
                ...
                <div>
                  ...
                  <Separator className="my-2" />
                  <div className="flex justify-between items-center py-1 font-semibold">
                    <span>Total Liabilities</span>enter py-1">
                    <span className="font-mono">{formatCurrency(balanceSheet.totalLiabilities)}</span>
                    <Separator className="my-2" />
                    <div className="flex justify-between items-center py-1 font-semibold">
                      <span>Total Liabilities</span>
n className="font-mono">{formatCurrency(balanceSheet.liabilities)}</span>
          ...
          <div>
            ...
            <div className="space-y-6">
              ...
              <div>
                  ...
                  <Separator className="my-2" />
                  <div className="flex justify-between items-center py-1 font-semibold">
                    <span>Total Revenue</span>-center py-1">
              ...
              {flowOfFunds.rebalanceVolume > 0 && (
                  ))}between items-center py-2 px-4 bg-muted/30 rounded-lg">
                  <Separator className="my-2" />me</span>
                  <div className="flex justify-between items-center py-1 font-semibold">
...
-3 gap-4">
          ...
          <div>
            <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
              ...
              Flow of Fundsperformance.return30d >= 0 ? 'text-green-500' : 'text-red-500'}`}>
            </h3>
            ...
            <div className="space-y-3">
              {flowOfFunds.flows.map((flow, idx) => (
                ...
                </div>-2xl font-bold ${performance.return90d >= 0 ? 'text-green-500' : 'text-red-500'}`}>
              ))}ormance.return90d)}
              <Separator className="my-2" />
              <div className="flex justify-between items-center py-3 px-4 bg-accent/10 border border-accent/30 rounded-lg">
                ...
                <span className={`font-mono font-bold text-lg ${flowOfFunds.netFlow >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                  {flowOfFunds.netFlow >= 0 ? '+' : ''}{formatCurrency(flowOfFunds.netFlow)}
                </span>e="text-xs">1 Year Return</CardDescription>
              </div>
              <div className="text-xs text-muted-foreground text-right">
                Period: {flowOfFunds.period}eturn1y >= 0 ? 'text-green-500' : 'text-red-500'}`}>
            </div>          </div>          {performance && (            <>              <Separator />              <div>                <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">                  <TrendUp size={20} className="text-accent" weight="fill" />                  Performance History                </h3>                                <div className="grid grid-cols-3 gap-4">                  <Card>                    <CardHeader className="pb-3">                      <CardDescription className="text-xs">30 Day Return</CardDescription>                    </CardHeader>                    <CardContent>                      <div className={`text-2xl font-bold ${performance.return30d >= 0 ? 'text-green-500' : 'text-red-500'}`}>                        {formatPercent(performance.return30d)}                      </div>                    </CardContent>                  </Card>                  <Card>                    <CardHeader className="pb-3">                      <CardDescription className="text-xs">90 Day Return</CardDescription>                    </CardHeader>                    <CardContent>                      <div className={`text-2xl font-bold ${performance.return90d >= 0 ? 'text-green-500' : 'text-red-500'}`}>                        {formatPercent(performance.return90d)}                      </div>                    </CardContent>                  </Card>                  <Card>                    <CardHeader className="pb-3">                      <CardDescription className="text-xs">1 Year Return</CardDescription>                    </CardHeader>                    <CardContent>                      <div className={`text-2xl font-bold ${performance.return1y >= 0 ? 'text-green-500' : 'text-red-500'}`}>                        {formatPercent(performance.return1y)}                      </div>                    </CardContent>                  </Card>              </div>            </>          )}          {notes && notes.length > 0 && (            <>              <Separator />              <div>                <h3 className="text-lg font-semibold mb-4">Notes & Disclosures</h3>                <div className="space-y-3">                  {notes.map((note, idx) => (                    <div key={idx} className="flex gap-3 p-3 bg-muted/30 rounded-lg">                      <div className="text-xs text-muted-foreground font-mono mt-0.5">{idx + 1}.</div>                      <p className="text-sm text-muted-foreground leading-relaxed flex-1">{note}</p>                    </div>                  ))}            </>          )}        </CardContent>      </Card>